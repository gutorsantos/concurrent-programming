...
void *f_cliente(void *v)
{
  ...
  if (sem_trywait(&sem_cadeiras) == 0) { 
    ...
    printf("O barbeiro está cortando o cabelo do cliente %d\n", id);
    // aguardar o corte do seu cabelo
    sem_wait(&corte);

    // liberar a cadeira do barbeiro
    sem_post(&cadeira_principal);
...
int main()
{
  ...
  sem_init(&cadeira_principal, 0, 1);
